...
@section('contenido')
<div class="container">
    <h1 id="titulo" >Editar PERSONA</h1>
    <form method="POST" action="{{route("Persona.update",$persona->DNI)}}">
        @method('PUT')
        @csrf
        <div class="form-group">
            <label class="control-label">N° DNI</label>
            <input type="text" style="color: blue"  class="form-control @error('dni') is-invalid @enderror"
                placeholder="Ingrese DNI" value="{{$persona->DNI}}" name="dni" disabled>
        </div>
        <div class="form-group">
            <label class="control-label">Apellido Paterno</label>
            <input type="text" class="form-control @error('apellido1') is-invalid @enderror" 
                placeholder="Ingrese Apellido paterno"  value="{{$persona->Apellido_Paterno}}" name="Apellido1">
            @error('apellido1')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control @error('apellido2') is-invalid @enderror" 
                placeholder="Ingrese Apellido Materno" value="{{$persona->Apellido_Materno}}" name="Apellido2">
            @error('apellido2')
                <span class="invalid feedback" role="alert">
                    <strong>{{$message}}</strong>
                </span>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control @error('nombres') is-invalid @enderror" 
                value="{{$persona->Nombres}}" name="nombres">
            @error('nombres')
            <span class="invalid feedback" role="alert">
                <strong>{{$message}}</strong>
            </span>
